...
{\color{red}{PDC errors in temperature (and velocity) tendencies occur because the $T$-increment (call it $\Delta T$) that the parameterizations prescribe leads to a dry thermal energy change of $\Delta M^{(d)}\Delta T$ which will not match the dry thermal energy change when the temeprature tendency is added in smaller chunks in the dynamical core during the `dribbling' of  $\Delta T$. The reason being that $\Delta M^{(d)}$ changes during each time-step and hence the thermal energy change due to physics forcing accumulated during the `dribbling' will not equal $\Delta M^{(d)}\Delta T$. This error could possibly be eliminated by using thermal energy forcing instead of temperature increments.   [mention that PDC errors can be divided into mass `clipping' errors and internal energy and kinetic energy.]}}
