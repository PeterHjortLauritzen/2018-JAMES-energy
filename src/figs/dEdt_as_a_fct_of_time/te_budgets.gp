...
set terminal pdf
set out "../dEdt.pdf"
set multiplot layout 1,2 title "{/Symbol \266}E model configuration 1 (TE consistent)" font ",30"
set grid
set yrange [-2.8:4.5]
set ylabel "{/Symbol \266}E [W/(m^2*s]"
set xlabel "time [months]"
set key top right spacing 2 maxrows 3
plot "te_budgets.txt" u 1 title "{/Symbol \266}E@_{phys}^{(efix)}" w l lw 1.3 lt 1,"te_budgets.txt" u 2 title "{/Symbol \266}E@_{phys}^{(param)}" w l lw 1.3 lt 2,"te_budgets.txt" u 3 title "{/Symbol \266}E@_{phys}^{(pwork)}" w l lw 1.3 lt 3,"te_budgets.txt" u 4 title "{/Symbol \266}E@_{dyn}^{(adiab)}" w l lw 1.3 lt 4,"te_budgets.txt" u (-$3-$4) title "-{/Symbol \266}E@_{phys}^{(pwork)}-{/Symbol \266}E@_{dyn}^{(adiab)}" w p pt 3 lt 7
set yrange [-0.7:0.85]
